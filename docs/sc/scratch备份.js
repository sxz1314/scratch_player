...
function like(){//点赞
	if (!isLogin){ layer.msg('请先登录'); return; }
	AjaxFn('/scratch/play/like', {pid:_pid},function (res) {
		layer.msg(res['msg']);
		if ('1' == res['status']){
			like_count = like_count + res['opt'];
			if (res['opt']==1){
				txt = `已赞(${like_count})`;
			} else {
				txt = `点赞(${like_count})`;
...
function reco(){
	if (!isLogin){ layer.msg('请先登录'); return; }
	AjaxFn('/scratch/play/reco', {pid:_pid},function (res) {
		layer.msg(res['msg']);
		if ('1' == res['status']){
			$('#reco').text(res['t']);	
		}
...
var assetUrlParts = ['https://www.comecode.net/', 'scratch/play/project/', assetIdParts[0]];
if (assetIdParts[1]) {assetUrlParts.push(assetIdParts[1]);}
return assetUrlParts.join('');
};
//项目资源文件下载URL
function getAssetUrl(asset) {
var assetUrlParts = ['https://www.comecode.net/', 'scratch/asset/', asset.assetId, '.', asset.dataFormat];
return assetUrlParts.join('');
};
...
const runBenchmark = function () {
    ...
    storage.webHelper.load = function (...args) {
        ...
        result.then(() => {
            ...
            switch(complete){
                case 1:{progress.textContent += ' 假装成真也有点难哦';break;}
                ...
                default:{progress.textContent += ' 如果网速够快，你是看不清我的留言的，哈哈！';}
            }
        });
        return result;
    };
